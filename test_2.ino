...
void setup() {
 ...
 // pinMode(13,OUTPUT);
  ...
  for (int i = 0; i < 3; ++i) // Open files to log processes
  {
    ...
    file_name +=".txt"; 
    file_name_2 =  "charge_battery_bench"; 
    file_name_2 += String(i+1) ;
    file_name_2 += "_" ;
    file_name_2 += String(now.day()) ;
    file_name_2 += "_" ;
...
double temperatures(int THERMISTORPIN){ // Ta mal a temperatura

// Colocar calculos da temperatura aqui
  double reading,aux,aux2;
  reading = analogRead(THERMISTORPIN); 

  
 aux = 0.0048875855327*reading;
  Serial.print("Volt");
  Serial.println(aux);
  aux2 = (10000*aux);

  aux2 = (aux2/(5-aux));
  Serial.print("Resist T");
  Serial.println(aux2);
  reading= -26.86*log(aux2) + 277.48;
 Serial.print("Temp"); 
  return  reading;
}
